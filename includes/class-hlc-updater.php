<?php
/**
 * GitHub-based auto-updater for Hostlinks Certificate Generator.
 *
 * How it works:
 *  1. On every WP update check this class calls the GitHub Releases API for
 *     spkldbrd/hostlinks-certificate-generator and caches the response for 12 hours.
 ...
 *  3. Cache is busted after a successful upgrade so the next check is fresh.
 *
 * To ship a new version:
 *  - Bump HLC_VERSION in hostlinks-certificate-generator.php
 *  - Commit and push to main — GitHub Actions builds
 *    hostlinks-certificate-generator.zip and creates / updates the Release.
 */
...
class HLC_Updater {
	...
	public function __construct( string $plugin_file, string $github_user, string $github_repo ) {
		...
		$this->transient_key = 'hlc_github_update_' . md5( $this->api_url );
	...
	public function check_for_update( $transient ) {
		...
		$current_version = $transient->checked[ $this->plugin_slug ] ?? HLC_VERSION;
	...
	public function plugin_info( $result, $action, $args ) {
		...
		return (object) array(
			'name'              => 'Hostlinks Certificate Generator',
			'slug'              => dirname( $this->plugin_slug ),
			'version'           => $version,
			...
			'tested'            => '',
			'last_updated'      => '',
			'short_description' => 'Generate and email PDF completion certificates from Hostlinks events.',
			'download_link'     => $download_link,
			'banners'           => array(),
			'icons'             => array(),
			'sections'          => array(
				'description' => 'Generate and email PDF completion certificates from Hostlinks events.',
